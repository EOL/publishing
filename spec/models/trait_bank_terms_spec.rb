...
RSpec.describe TraitBank::Glossary do
  ...
  describe ".count" do
    it "caches the value" do
      ...
      expect(TraitBank::Glossary.count).to eq(:foo)
    end

    it "queries TraitBank" do
      expect(TraitBank::Glossary).to receive(:query).
  ...
  describe ".full_glossary" do
    it "caches the value" do
      ...
      expect(TraitBank::Glossary.full_glossary).to eq(:this)
    end
    ...
    it "queries TraitBank" do
      ...
      expect(TraitBank::Glossary.full_glossary).to eq([{ foo: "bar" }])
    end
  end
  ...
  describe ".sub_glossary" do
    it "caches the value" do
      expect(Rails.cache).to receive(:fetch) { :this }
      expect(TraitBank::Glossary.sub_glossary(:which)).to eq(:this)
    end
    ...
    it "queries TraitBank" do
      ...
      expect(TraitBank::Glossary.sub_glossary(:which)).to eq([{ foo: "bar" }])
    end
    ...
    it "can count" do
      ...
      expect(TraitBank::Glossary.sub_glossary(:which, 1, 1, count: true)).to eq(:this)
    end
  end
end
