...
#     
    # it "should redirect to resource on successful url" do
      # put :update ,resource: FactoryGirl.attributes_for(:resource , :valid_url) , content_partner_id: 1 
      # #response.should redirect_to controller: 'resources', action: 'show', id: result
    # end
#     
    # it "should redirect to resource on successful file upload" do
      # put :update ,resource: FactoryGirl.attributes_for(:resource , :valid_file) , content_partner_id: 1 
      # #response.should redirect_to controller: 'resources', action: 'show', id: result
    # end
#     
    # it "should re-render new template on failed url" do
      # put :update ,resource: FactoryGirl.attributes_for(:resource , :invalid_url) , content_partner_id: 1 
      # expect(response.body).to render_template(:edit)
    # end
#     
    # it "should re-render new template on failed file upload" do
      # put :update ,resource: FactoryGirl.attributes_for(:resource , :invalid_file) , content_partner_id: 1 
      # expect(response.body).to render_template(:edit)
    # end
#     
    it "should re-render new template on failed save" do
      put :update ,resource: FactoryGirl.attributes_for(:resource , :invalid) , content_partner_id: 1 
      expect(response.body).to render_template(:edit)
    end
  end
end