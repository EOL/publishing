  ...
  class << self
    def discourse
      ...
      return Rails.logger.error("Missing discourse_url") if
        Rails.application.credentials.discourse_key.nil?
      return Rails.logger.error("Missing discourse_url") if
        Rails.application.credentials.discourse_user.nil?
      @client = DiscourseApi::Client.new(
        discourse_url,
        Rails.application.credentials.discourse_key,
        Rails.application.credentials.discourse_user
      )
    end

    def discourse_url
      @discourse_url ||= Rails.application.credentials.discourse_url
    end
    ...
    def delete_empty_comment_topics
      ...
      empty_comment_topics.each do |topic|
        ...
        #   { status: 'archived', enabled: true, api_username: Rails.application.credentials.discourse_user })
        count += 1
      end
      count
    end
