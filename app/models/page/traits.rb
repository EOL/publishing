...
class Page
  module Traits
    ...
    def first_trait_for_object_terms(object_terms, options = {})
      ...
      TraitBank::Corruption.guard(id, fallback: nil) do
    ...
    def traits_for_predicates_helper(predicates, trait_match, options = {})
      empty = options[:limit].present? ? nil : []
      return empty if page_node.nil?
      ...
      TraitBank::Corruption.guard(id, fallback: empty) do
        ...
        if options[:return_predicate]
          Trait.populate_pk_result(q.return('trait.eol_pk AS trait_pk, parent_predicate AS predicate'), includes: includes, page_id: id)
        elsif options[:limit]
          Trait.wrap_node(
            q.return(:trait)
            .limit(options[:limit])
          ...
          Trait.for_eol_pks(q.pluck('trait.eol_pk'), includes: includes, page_id: id)
        end
      end
    end
