...
class FetchRelationshipsJob
  def perform
    ...
    Delayed::Worker.logger.info("Fetching Parent/child relationships")
    count = TraitBank::Terms::Relationships.fetch_parent_child_relationships(log)
    Delayed::Worker.logger.info("Completed fetch of #{count} Parent/child relationships")
...
class FetchSynonymsJob
  def perform
    log = TraitBank::Terms::FetchLog.new
    Delayed::Worker.logger.info("Fetching Synonyms")
    count = TraitBank::Terms::Relationships.fetch_synonyms(log)
    Delayed::Worker.logger.info("Completed fetch of #{count} Synonyms")
...
class FetchUnitsJob
  def perform
    log = TraitBank::Terms::FetchLog.new
    Delayed::Worker.logger.info("Fetching Unit Terms")
    count = TraitBank::Terms::Relationships.fetch_units(log)
    Delayed::Worker.logger.info("Completed fetch of #{count} Unit Terms")
    log << "Loaded #{count} predicate/unit relationships."
  end
