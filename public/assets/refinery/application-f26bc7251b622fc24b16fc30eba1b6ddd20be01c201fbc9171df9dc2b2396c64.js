...
trigger_reordering = function(e, enable) {
  ...
  $('#site_bar, #content').fadeTo(0, enable ? 0.35 : 1);

  if(enable) {
    $menu.find('.tab a').on('click', function(ev){
      ev.preventDefault();
    });
  } else {
  ...
  $.post(
    ...
    , function(data) {
      if (($flash_container = $('#flash_container')).length > 0) {
        ...
        //updates the form action and locale picker links with new URLs
        $('form').attr('action', $('#new_action').attr('value'));
        $('#switch_locale_picker > li > a').each(function(i, anchor) {
          ($anchor = $(anchor)).attr('href',
            $('#new_refinery_edit_page_path').attr('value')
              + "?switch_locale="
              + $anchor.attr('href').split('?switch_locale=').slice(-1)[0]
          );
        });
      }
    }, 'html'
    ...
    $(element).hover(function(e) {
      if (e.type == 'mouseenter' || e.type == 'mouseover') {
        $(this).oneTime(350, 'tooltip', $.proxy(function() {
          ...
          tooltip_nib_image_src = $.browser.msie ? "/assets/refinery/tooltip-nib-6280314d60d0e6a22c44657fd0aece927f410165b283ad26dd76ab402f5674a0.gif" : "/assets/refinery/tooltip-nib-24efe774607e3c73edab7d0a9ea3d099fa5917441df307fb957a99987f30652e.png";
...
var link_tester = {
  ...
  validate_textbox: function(validation_method, textbox_id, callback) {
    var icon = '';
    var loader_img = $("<img id='" + textbox_id.replace('#','') + "_test_loader' src='/assets/refinery/ajax-loader-b9f6dd5edd1222b6fddbc439d64797ef90c669043cb1996a6598f1966563656a.gif' alt='Testing...' style='display: none;'/>");
  ...
  $('nav#actions.multilist > ul:not(.search_list) li > a').not('.not_a_link a').not('a.reorder_icon').each(function(i,a){
    if ($(this).data('dialog-title') == null) {
      $(this).bind('click', function(){
        $(this).css('background-image', "url('/assets/refinery/ajax-loader-b9f6dd5edd1222b6fddbc439d64797ef90c669043cb1996a6598f1966563656a.gif') !important");
      });
    }
  });
